...
// ── Formulas ──────────────────────────────────────────────────
function calcTDEE({ age, weight, height, sex, activity_factor, goal }) {
  if (!age || !weight || !height) return null
  const bmr =
    sex === 'H'
      ? 10 * weight + 6.25 * height - 5 * age + 5
      : 10 * weight + 6.25 * height - 5 * age - 161
  const tdee = bmr * (activity_factor || 1.375)
...
function calcMacros(kcalTarget, goal) {
  ...
  return {
    ...
    g: Math.round((kcalTarget * (1 - pRatio - lRatio)) / 4),
...
{"description":"Nom court du plat","items":[{"name":"Aliment (portion)","kcal":0,"p":0.0,"g":0.0,"l":0.0}],"total":{"kcal":0,"p":0.0,"g":0.0,"l":0.0},"confidence":"haute|moyenne|faible","note":"Remarque courte"}`
    ...
    <div style={{ marginBottom: 12 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
        ...
        <span style={{ color: COLORS.text, fontSize: 12, fontWeight: 600 }}>
          {Math.round(value)}g <span style={{ color: COLORS.dimmed }}>/ {max}g</span>
        </span>
      </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      <svg width={140} height={140} viewBox="0 0 140 140">
        ...
        <circle cx={70} cy={70} r={r} fill="none" stroke={isOver ? COLORS.coral : COLORS.mint}
        ...
        <text x={70} y={62} textAnchor="middle" fill={COLORS.text} fontSize={22} fontWeight={700}>{Math.round(net)}</text>
      ...
      <div style={{ display: 'flex', gap: 20, fontSize: 12 }}>
        {[{ v: Math.round(consumed), label: 'Consommé', c: COLORS.amber }, { v: Math.round(burned), label: 'Brûlé', c: COLORS.mint }, { v: goal, label: 'Objectif', c: COLORS.dimmed }].map((s) => (
    ...
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, height: 80, padding: '0 4px' }}>
      {data.map((d, i) => (
        <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
          <div title={`${d.net} kcal`} style={{ width: '100%', height: Math.max((d.net / maxVal) * 70, 3), background: d.today ? COLORS.mint : d.net > goal ? COLORS.coral : COLORS.border, borderRadius: 3, transition: 'height 0.3s ease' }} />
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: `1px solid ${COLORS.border}` }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        <input type="number" value={value} min={min} max={max} onChange={(e) => onChange(e.target.value)}
          style={{ width: 70, background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: '6px 10px', color: COLORS.text, fontSize: 14, fontWeight: 600, textAlign: 'right' }} />
...
function IMCBar({ bmi }) {
  const zones = [{ label: 'Sous-poids', max: 18.5, color: '#74B9FF' }, { label: 'Normal', max: 25, color: COLORS.mint }, { label: 'Surpoids', max: 30, color: COLORS.amber }, { label: 'Obésité', max: 40, color: COLORS.coral }]
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
        ...
        <span style={{ fontSize: 13, fontWeight: 700, color: zone.color }}>{bmi.toFixed(1)} — {zone.label}</span>
      </div>
      <div style={{ position: 'relative', height: 10, borderRadius: 6, background: `linear-gradient(to right, #74B9FF 0%, ${COLORS.mint} 28%, ${COLORS.amber} 60%, ${COLORS.coral} 100%)` }}>
        <div style={{ position: 'absolute', top: -3, left: `${((clamp - 10) / 30) * 100}%`, transform: 'translateX(-50%)', width: 16, height: 16, borderRadius: '50%', background: COLORS.text, border: `3px solid ${zone.color}`, transition: 'left 0.4s ease' }} />
      </div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
        {['10', '18.5', '25', '30', '40'].map((v) => <span key={v} style={{ fontSize: 9, color: COLORS.dimmed }}>{v}</span>)}
      </div>
    </div>
  )
}
...
function ConfidenceBadge({ level }) {
  const map = { haute: { color: COLORS.mint, label: '✓ Confiance haute' }, moyenne: { color: COLORS.amber, label: '~ Confiance moyenne' }, faible: { color: COLORS.coral, label: '! Confiance faible' } }
...
export default function App() {
  ...
  const [profile, setProfile] = useState({ name: '', age: 30, weight: 70, height: 175, sex: 'H', activity_factor: 1.375, goal: 'Maintien' })
  const [profileSaving, setProfileSaving] = useState(false)

  // Daily data from Supabase
  ...
  const loadData = useCallback(async () => {
    ...
    const [{ data: prof }, { data: mealRows }, { data: actRows }] = await Promise.all([
    ...
    if (mealRows) setMeals(mealRows)
    if (actRows) setActivities(actRows)
  ...
  const macroTargets = useMemo(() => tdeeData ? calcMacros(tdeeData.target, profile.goal) : { p: 150, g: 250, l: 70 }, [tdeeData, profile.goal])
  const kcalGoal = tdeeData?.target || 2000
  const consumed = useMemo(() => meals.reduce((s, m) => s + Number(m.kcal) * Number(m.qty), 0), [meals])
  const burned = useMemo(() => activities.reduce((s, a) => s + Number(a.kcal), 0), [activities])
  const totalP = useMemo(() => meals.reduce((s, m) => s + Number(m.p) * Number(m.qty), 0), [meals])
  const totalG = useMemo(() => meals.reduce((s, m) => s + Number(m.g) * Number(m.qty), 0), [meals])
  const totalL = useMemo(() => meals.reduce((s, m) => s + Number(m.l) * Number(m.qty), 0), [meals])
  const weekData = useMemo(() => {
    const fake = [1420, 1870, 1650, 2100, 1980, 1750, Math.max(consumed - burned, 0)]
    return fake.map((net, i) => ({ net, today: i === 6 }))
  }, [consumed, burned])
  const bmi = useMemo(() => profile.weight && profile.height ? profile.weight / (profile.height / 100) ** 2 : null, [profile.weight, profile.height])
  ...
  const mealGroups = MEALS.map((m) => ({
    ...
    total: meals.filter((x) => x.meal === m).reduce((s, x) => s + Number(x.kcal) * Number(x.qty), 0),
  }))
  ...
  const updProfile = (key, val) => setProfile((p) => ({ ...p, [key]: val }))

  // ── Meal operations ──────────────────────────────────────────
  const addMeal = async () => {
    if (!selFood || !user) return
    const row = { user_id: user.id, meal: selMeal, name: selFood.name, kcal: selFood.kcal, p: selFood.p, g: selFood.g, l: selFood.l, qty, from_photo: false, date: todayISO() }
    const { data } = await supabase.from('meals').insert(row).select().single()
    if (data) setMeals((prev) => [...prev, data])
    setShowFood(false); setFoodSearch(''); setSelFood(null); setQty(1)
  }
  ...
  const addPhotoMeals = async () => {
    if (!analysisResult || !user) return
    const rows = analysisResult.items
      .filter((_, i) => selectedItems.includes(i))
      .map((item) => ({ user_id: user.id, meal: selMeal, name: item.name, kcal: item.kcal, p: item.p, g: item.g, l: item.l, qty: 1, from_photo: true, date: todayISO() }))
  ...
  const addActivity = async () => {
    if (!selActivity || !user) return
    const row = { user_id: user.id, name: selActivity.name, icon: selActivity.icon, duration, kcal: Math.round(selActivity.kcalPerMin * duration), date: todayISO() }
    const { data } = await supabase.from('activities').insert(row).select().single()
    if (data) setActivities((prev) => [...prev, data])
    setShowActivity(false); setSelActivity(null); setDuration(30)
  }
  ...
  const handlePhotoSelect = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      setPhotoPreview(dataUrl); setPhotoBase64(dataUrl.split(',')[1]); setPhotoMediaType(file.type)
      setAnalysisResult(null); setAnalysisError(null); setSelectedItems([])
    }
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      setAnalysisResult(result); setSelectedItems(result.items.map((_, i) => i))
    } catch { setAnalysisError('Analyse impossible. Vérifiez votre connexion.') }
  ...
  const closePhotoModal = () => {
    setShowFood(false); setPhotoPreview(null); setPhotoBase64(null); setPhotoMediaType(null)
    setAnalysisResult(null); setAnalysisError(null); setSelectedItems([]); setFoodMode('search')
  }

  // ── Logout ────────────────────────────────────────────────────
  const handleLogout = async () => {
    await supabase.auth.signOut()
    setMeals([]); setActivities([])
    setProfile({ name: '', age: 30, weight: 70, height: 175, sex: 'H', activity_factor: 1.375, goal: 'Maintien' })
  }
  ...
  // ── Shared style ──────────────────────────────────────────────
  const card = { background: COLORS.card, borderRadius: 14, border: `1px solid ${COLORS.border}`, padding: 16 }
    ...
    <div style={{ fontFamily: "'Inter', system-ui, sans-serif", background: COLORS.bg, minHeight: '100vh', color: COLORS.text, maxWidth: 420, margin: '0 auto', paddingBottom: 80 }}>
      ...
      <div style={{ padding: '20px 16px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          {profile.name && <div style={{ background: COLORS.mint, color: COLORS.bg, borderRadius: 20, padding: '4px 14px', fontSize: 13, fontWeight: 700 }}>{profile.name}</div>}
          <button onClick={handleLogout} title="Déconnexion" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 10, padding: '6px 10px', color: COLORS.muted, fontSize: 12, cursor: 'pointer' }}>
            ⎋ Quitter
          </button>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', margin: '16px 16px 0', background: COLORS.surface, borderRadius: 10, padding: 3 }}>
        {[['profile', '👤 Profil'], ['dashboard', '📊 Bilan'], ['food', '🍽 Repas'], ['activity', '🏃 Sport']].map(([key, label]) => (
          <button key={key} onClick={() => setTab(key)} style={{ flex: 1, padding: '8px 2px', borderRadius: 8, border: 'none', background: tab === key ? COLORS.mint : 'transparent', color: tab === key ? COLORS.bg : COLORS.muted, fontSize: 11, fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s' }}>
            {label}
          </button>
        ))}
      </div>
        ...
        <div style={{ padding: '16px 16px 0' }}>
          ...
          {tab === 'profile' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div style={{ ...card, fontSize: 12, color: COLORS.muted, padding: '10px 16px' }}>
                Connecté en tant que <span style={{ color: COLORS.mint, fontWeight: 600 }}>{user.email}</span>
              </div>
              ...
              <div style={card}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: `1px solid ${COLORS.border}` }}>
                  ...
                  <input value={profile.name || ''} onChange={(e) => updProfile('name', e.target.value)} placeholder="Votre prénom"
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: `1px solid ${COLORS.border}` }}>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    {['H', 'F'].map((s) => (
                      <button key={s} onClick={() => updProfile('sex', s)} style={{ padding: '6px 18px', borderRadius: 20, border: 'none', background: profile.sex === s ? COLORS.mint : COLORS.border, color: profile.sex === s ? COLORS.bg : COLORS.muted, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                        {s === 'H' ? 'Homme' : 'Femme'}
                      </button>
                    ))}
                  </div>
                </div>
                <InputRow label="Âge" unit="ans" value={profile.age || ''} min={10} max={100} onChange={(v) => updProfile('age', Number(v))} />
                <InputRow label="Poids" unit="kg" value={profile.weight || ''} min={30} max={300} onChange={(v) => updProfile('weight', Number(v))} />
                <InputRow label="Taille" unit="cm" value={profile.height || ''} min={100} max={250} onChange={(v) => updProfile('height', Number(v))} />
              </div>
              ...
              <div style={card}>
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  {GOALS.map((g) => (
                    <button key={g} onClick={() => updProfile('goal', g)} style={{ flex: 1, padding: '10px 8px', borderRadius: 10, border: `1px solid ${profile.goal === g ? COLORS.mint : COLORS.border}`, background: profile.goal === g ? `${COLORS.mint}20` : 'transparent', color: profile.goal === g ? COLORS.mint : COLORS.muted, fontSize: 12, fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s' }}>
                      {g}
                    </button>
                  ))}
                </div>
              ...
              <div style={card}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {ACTIVITY_LEVELS.map((a) => (
                    <div key={a.label} onClick={() => updProfile('activity_factor', a.factor)}
                      style={{ padding: '10px 12px', borderRadius: 10, border: `1px solid ${profile.activity_factor === a.factor ? COLORS.mint : COLORS.border}`, background: profile.activity_factor === a.factor ? `${COLORS.mint}15` : 'transparent', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', transition: 'all 0.2s' }}>
                      <div>
                        <div style={{ fontSize: 13, fontWeight: 600, color: profile.activity_factor === a.factor ? COLORS.mint : COLORS.text }}>{a.label}</div>
              ...
              {tdeeData && (
                <div style={card}>
                  ...
                  <button onClick={saveProfile} disabled={profileSaving} style={{ marginTop: 14, width: '100%', background: profileSaving ? COLORS.border : COLORS.mint, color: profileSaving ? COLORS.dimmed : COLORS.bg, border: 'none', borderRadius: 12, padding: '14px', fontSize: 14, fontWeight: 700, cursor: profileSaving ? 'default' : 'pointer' }}>
          ...
          {tab === 'dashboard' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {[{ label: 'Apport', value: `${Math.round(consumed)} kcal`, icon: '🍽', color: COLORS.amber }, { label: 'Dépense', value: `${Math.round(burned)} kcal`, icon: '🔥', color: COLORS.mint }].map((s) => (
          ...
          {tab === 'food' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <button onClick={() => { setFoodMode('search'); setShowFood(true) }} style={{ background: COLORS.card, color: COLORS.text, border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: '14px 10px', fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                  <span style={{ fontSize: 24 }}>🔍</span>Rechercher
                </button>
                <button onClick={() => { setFoodMode('photo'); setShowFood(true) }} style={{ background: `linear-gradient(135deg, ${COLORS.mint}22, ${COLORS.amber}22)`, color: COLORS.mint, border: `1px solid ${COLORS.mint}55`, borderRadius: 12, padding: '14px 10px', fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                  <span style={{ fontSize: 24 }}>📸</span>Photo IA
                </button>
              </div>
              {mealGroups.map((g) => (
                <div key={g.label} style={card}>
                  ...
                  {g.items.length === 0 ? (
                    ...
                    <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderTop: `1px solid ${COLORS.border}` }}>
                      <div>
                        <div style={{ fontSize: 13, display: 'flex', alignItems: 'center', gap: 4 }}>
                          {item.from_photo && <span style={{ fontSize: 10, background: `${COLORS.mint}22`, color: COLORS.mint, borderRadius: 4, padding: '1px 4px', fontWeight: 600 }}>IA</span>}
                          {item.name}
                        </div>
                        <div style={{ fontSize: 11, color: COLORS.muted }}>×{item.qty} · P:{Math.round(item.p * item.qty)}g G:{Math.round(item.g * item.qty)}g L:{Math.round(item.l * item.qty)}g</div>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span style={{ color: COLORS.amber, fontSize: 13, fontWeight: 600 }}>{Math.round(item.kcal * item.qty)}</span>
                        <button onClick={() => removeMeal(item.id)} style={{ background: 'none', border: 'none', color: COLORS.dimmed, cursor: 'pointer', fontSize: 16, padding: 0 }}>×</button>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {tab === 'activity' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <button onClick={() => setShowActivity(true)} style={{ background: COLORS.mint, color: COLORS.bg, border: 'none', borderRadius: 12, padding: '14px', fontSize: 14, fontWeight: 700, cursor: 'pointer', width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
              ...
              {activities.length === 0 ? (
                ...
                <div style={card}>
                  ...
                  {activities.map((a, i) => (
                    <div key={a.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderTop: i > 0 ? `1px solid ${COLORS.border}` : 'none' }}>
                      <div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        <button onClick={() => removeActivity(a.id)} style={{ background: 'none', border: 'none', color: COLORS.dimmed, cursor: 'pointer', fontSize: 16, padding: 0 }}>×</button>
                      </div>
                    </div>
                  ))}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(10,18,28,0.85)', zIndex: 100, display: 'flex', alignItems: 'flex-end' }} onClick={() => setShowFood(false)}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: COLORS.card, borderRadius: '20px 20px 0 0', width: '100%', maxWidth: 420, margin: '0 auto', padding: 20, maxHeight: '80vh', display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {MEALS.map((m) => <button key={m} onClick={() => setSelMeal(m)} style={{ padding: '6px 12px', borderRadius: 20, border: 'none', background: selMeal === m ? COLORS.mint : COLORS.border, color: selMeal === m ? COLORS.bg : COLORS.muted, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>{m}</button>)}
            </div>
            <input placeholder="Rechercher…" value={foodSearch} onChange={(e) => { setFoodSearch(e.target.value); setSelFood(null) }}
            ...
            <div style={{ overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: 6 }}>
              {filteredFoods.map((f, i) => (
                <div key={i} onClick={() => setSelFood(f)} style={{ padding: '10px 12px', borderRadius: 10, background: selFood === f ? COLORS.surface : 'transparent', border: `1px solid ${selFood === f ? COLORS.mint : COLORS.border}`, cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
      ...
      {showFood && foodMode === 'photo' && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(10,18,28,0.9)', zIndex: 100, display: 'flex', alignItems: 'flex-end' }} onClick={closePhotoModal}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: COLORS.card, borderRadius: '20px 20px 0 0', width: '100%', maxWidth: 420, margin: '0 auto', padding: 20, maxHeight: '90vh', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            {!photoPreview ? (
              <div onClick={() => fileInputRef.current?.click()} style={{ border: `2px dashed ${COLORS.mint}66`, borderRadius: 14, padding: '40px 20px', textAlign: 'center', cursor: 'pointer', background: `${COLORS.mint}08` }}>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <button onClick={() => { setPhotoPreview(null); setPhotoBase64(null); setAnalysisResult(null); setAnalysisError(null) }}
                  style={{ position: 'absolute', top: 8, right: 8, background: 'rgba(0,0,0,0.6)', border: 'none', borderRadius: '50%', width: 28, height: 28, color: '#fff', cursor: 'pointer', fontSize: 14 }}>×</button>
              </div>
            )}
            {photoPreview && !analysisResult && !analyzing && (
              <button onClick={handleAnalyze} style={{ background: `linear-gradient(135deg, ${COLORS.mint}, #2BA898)`, color: COLORS.bg, border: 'none', borderRadius: 12, padding: '14px', fontSize: 14, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                ✨ Analyser avec l'IA
              </button>
            )}
            {analyzing && <Spinner label="Analyse en cours…" />}
            {analysisError && <div style={{ background: `${COLORS.coral}22`, border: `1px solid ${COLORS.coral}55`, borderRadius: 10, padding: '12px 14px', color: COLORS.coral, fontSize: 13 }}>{analysisError}</div>}
            {analysisResult && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                  {[{ l: 'Protéines', v: analysisResult.total.p, c: COLORS.mint }, { l: 'Glucides', v: analysisResult.total.g, c: COLORS.amber }, { l: 'Lipides', v: analysisResult.total.l, c: COLORS.coral }].map((m) => (
                ...
                {analysisResult.note && <div style={{ background: `${COLORS.amber}15`, border: `1px solid ${COLORS.amber}33`, borderRadius: 10, padding: '10px 12px', fontSize: 12, color: COLORS.amber }}>💡 {analysisResult.note}</div>}
                ...
                {analysisResult.items.map((item, i) => (
                  <div key={i} onClick={() => toggleItem(i)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 12px', borderRadius: 10, border: `1px solid ${selectedItems.includes(i) ? COLORS.mint : COLORS.border}`, background: selectedItems.includes(i) ? `${COLORS.mint}12` : 'transparent', cursor: 'pointer', transition: 'all 0.2s' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <div style={{ width: 20, height: 20, borderRadius: 6, border: `2px solid ${selectedItems.includes(i) ? COLORS.mint : COLORS.dimmed}`, background: selectedItems.includes(i) ? COLORS.mint : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                ...
                <button onClick={addPhotoMeals} disabled={selectedItems.length === 0} style={{ background: selectedItems.length > 0 ? COLORS.mint : COLORS.border, color: selectedItems.length > 0 ? COLORS.bg : COLORS.dimmed, border: 'none', borderRadius: 12, padding: '14px', fontSize: 14, fontWeight: 700, cursor: selectedItems.length > 0 ? 'pointer' : 'default' }}>
      ...
      {showActivity && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(10,18,28,0.85)', zIndex: 100, display: 'flex', alignItems: 'flex-end' }} onClick={() => setShowActivity(false)}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: COLORS.card, borderRadius: '20px 20px 0 0', width: '100%', maxWidth: 420, margin: '0 auto', padding: 20, maxHeight: '80vh', display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              {['Maison', 'Cardio', 'Sport'].map((group) => (
                <div key={group}>
                  ...
                  {ACTIVITY_DB.filter((a) => a.group === group).map((a, i) => (
                    <div key={i} onClick={() => setSelActivity(a)} style={{ padding: '11px 12px', marginBottom: 5, borderRadius: 10, border: `1px solid ${selActivity === a ? COLORS.mint : COLORS.border}`, background: selActivity === a ? `${COLORS.mint}15` : 'transparent', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button onClick={addActivity} disabled={!selActivity} style={{ background: selActivity ? COLORS.mint : COLORS.border, color: selActivity ? COLORS.bg : COLORS.dimmed, border: 'none', borderRadius: 12, padding: '14px', fontSize: 14, fontWeight: 700, cursor: selActivity ? 'pointer' : 'default' }}>
              Ajouter
            </button>
          </div>
        </div>
